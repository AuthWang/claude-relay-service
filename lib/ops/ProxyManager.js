...
server {
   ...
   * 检查Caddy服务是否运行
   */
  async isCaddyRunning() {
    try {
      const status = execSync('sudo systemctl is-active caddy', {
        encoding: 'utf8',
        stdio: 'pipe'
      }).trim();
  ...
  async restartProxy(proxyTool) {
    try {
      if (proxyTool === 'caddy') {
        ...
        // 检查Caddy是否已经运行
        const isRunning = await this.isCaddyRunning();

        if (isRunning) {
          console.log('🔄 重新加载Caddy配置...');
          execSync('sudo systemctl reload caddy', { stdio: 'inherit' });
        } else {
          console.log('🚀 启动Caddy服务...');
          execSync('sudo systemctl start caddy', { stdio: 'inherit' });
        }
  ...
  async verifyCaddyStatus() {
    try {
      ...
      if (status === 'active') {
        ...
        // 额外检查端口监听状态
        try {
          execSync('sudo ss -tlnp | grep :80 | grep caddy', { stdio: 'pipe' });
          console.log('✅ Caddy 已监听端口 80');
        } catch (error) {
          console.log('⚠️  Caddy 服务运行中但可能未监听端口 80');
        }

        try {
          execSync('sudo ss -tlnp | grep :443 | grep caddy', { stdio: 'pipe' });
          console.log('✅ Caddy 已监听端口 443');
        } catch (error) {
          console.log('⚠️  Caddy 服务运行中但可能未监听端口 443');
        }

      } else {
        console.error(`❌ Caddy服务状态异常: ${status}`);

        // 提供详细的错误信息
        try {
          const detailedStatus = execSync('sudo systemctl status caddy --no-pager -l', {
            encoding: 'utf8',
            stdio: 'pipe'
          });
          console.error('📊 详细状态信息:');
          console.error(detailedStatus);
        } catch (statusError) {
          console.error('❌ 无法获取详细状态信息');
        }
      ...
      console.error('💡 排查建议:');
      console.error('   1. 检查服务日志: sudo journalctl -xeu caddy.service');
      console.error('   2. 验证配置语法: sudo caddy validate --config /etc/caddy/Caddyfile');
      console.error('   3. 检查端口占用: sudo ss -tlnp | grep :80');
      console.error('   4. 手动启动测试: sudo caddy start --config /etc/caddy/Caddyfile');
